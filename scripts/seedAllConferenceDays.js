...
const seedAllDays = async () => {
  try {
    await mongoose.connect(MONGO_URI);
    console.log("Connected to MongoDB for Conference Days seeding...");

    for (const dayData of conferenceData) {
    ...
    console.log("All Conference Days seeded successfully.");
    process.exit(0);
  } catch (error) {
    console.error("Error seeding Conference Days:", error);
    process.exit(1);
  }
};
