...
async function trySendAisensyForFormType(formType, mobile, template, data) {
    ...
    if ((formType === 'corporate-visitor' || formType === 'international-visitor' || formType === 'general-visitor' || formType === 'buyer-registration' || formType === 'health-camp-visitor') && data.registrationId) {
        ...
        mediaUrl = `https://quickchart.io/qr?text=${encodeURIComponent(scanUrl)}&size=300&margin=2`;
        filename = `qrcode_${data.registrationId.replace(/\//g, '_')}.png`;
    }
    ...
    return await aisensy.sendTemplate({
        ...
        media: mediaUrl ? { url: mediaUrl, filename } : null
    });
}
