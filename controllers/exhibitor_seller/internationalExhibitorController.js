const internationalExhibitorService = require('../../services/exhibitor_seller/internationalExhibitorService');
const { logActivity } = require('../../utils/logger');

class InternationalExhibitorController {
    async getAllRegistrations(req, res) {
        try {
            const data = await internationalExhibitorService.getAllRegistrations();
    ...
    async getRegistrationById(req, res) {
        try {
            const data = await internationalExhibitorService.getRegistrationById(req.params.id);
    ...
    async register(req, res) {
        try {
            ...
            const saved = await internationalExhibitorService.addRegistration(formData);

            try {
                await logActivity(req, 'Created', 'International Exhibitor', `New registration: ${saved.brandName} (${saved.registrationId})`);
            } catch (logErr) {
    ...
    async updateStatus(req, res) {
        try {
            ...
            const updated = await internationalExhibitorService.updateRegistration(id, { 
    ...
    async deleteRegistration(req, res) {
        try {
            const data = await internationalExhibitorService.getRegistrationById(req.params.id);
            await internationalExhibitorService.deleteRegistration(req.params.id);
            
            if (data) {
                await logActivity(req, 'Deleted', 'International Exhibitor', `Deleted: ${data.brandName} (${data.registrationId})`);
            }
...
module.exports = new InternationalExhibitorController();
