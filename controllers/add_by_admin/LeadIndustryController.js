const LeadIndustry = require("../../models/add_by_admin/LeadIndustry");
...
const seedDefaults = async () => {
  const count = await LeadIndustry.countDocuments();
  if (count === 0) {
    const docs = DEFAULT_INDUSTRIES.map((name, idx) => ({
      name,
      order: idx + 1,
      status: "Active",
      added_by: "System",
    }));
    await LeadIndustry.insertMany(docs);
  }
};
...
const getAll = async (req, res) => {
  try {
    ...
    const records = await LeadIndustry.find(filter).sort({ order: 1, added: 1 });
...
const getById = async (req, res) => {
  try {
    const record = await LeadIndustry.findById(req.params.id);
...
const remove = async (req, res) => {
  try {
    const record = await LeadIndustry.findByIdAndDelete(req.params.id);
    if (!record) return res.status(404).json({ success: false, message: "Not found" });
