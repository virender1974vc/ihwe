...
exports.getConferenceDay = async (req, res) => {
  try {
    ...
    const dayData = await ConferenceDay.findOne({ dayNumber: parseInt(dayNumber) });

    if (!dayData) {
      return res.status(404).json({ success: false, message: `Conference data for Day ${dayNumber} not found` });
    }
