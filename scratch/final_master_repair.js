...
// Valid Header: Line 1 to 333
const cleanHead = lines.slice(0, 332).join('\n');

const middleHandlers = `
...
const cleanTail = \`
    return (
\${lines.slice(lines.findIndex(l => l.includes('return (')) + 1).join('\\n')}
\`;

// Actually, I'll find the LAST "return (" and the LAST "export default"
const lastReturnIdx = content.lastIndexOf('return (');
const lastExportIdx = content.lastIndexOf('export default BookAStand;');

const uiAndEnd = content.substring(lastReturnIdx, lastExportIdx + 'export default BookAStand;'.length);

const finalTSX = cleanHead + middleHandlers + uiAndEnd;

fs.writeFileSync(filePath, finalTSX);
console.log('Final Master Repair Complete.');
