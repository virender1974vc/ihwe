...
// The file is likely: [Valid Start] [Corrupted Summary] [Duplicate End] [Corrupted Summary] ...
// I'll find the first occurrence of the summary start and the last occurrence of the form end.

const summaryStart = '<div className="w-full">                                                            <div className="flex justify-between items-center mb-3">';
const formEnd = '</form>';

const firstSummaryIdx = content.indexOf(summaryStart);
const lastFormEndIdx = content.lastIndexOf(formEnd);

if (firstSummaryIdx !== -1 && lastFormEndIdx !== -1) {
    const validStart = content.substring(0, firstSummaryIdx);
    // Find the end of the component after the last form end
    const rest = content.substring(lastFormEndIdx);
    const componentEndMatch = rest.match(/[\s\S]+?export default BookAStand;/);
    const validEnd = componentEndMatch ? componentEndMatch[0] : rest;

    // The clean summary I want to inject
    ...
    const finalContent = validStart + cleanSummary + "\n                                                    " + validEnd;
    fs.writeFileSync(filePath, finalContent);
    console.log('Public BookAStand.tsx Summary RE-Synthesized.');
} else {
    console.log('Failing to find markers.');
}
