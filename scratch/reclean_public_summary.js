...
// I'll define the CLEAN summary block WITHOUT any excessive whitespace between tags.
const cleanSummaryBlock = `<div className="w-full">
...
// I'll find where the summary starts based on a shorter marker that is less likely to have NBSP issues
const markerA = '<div className="w-full">';
const markerB = '<h3 className="text-sm font-bold text-slate-900 uppercase">Booking Summary</h3>';

// I'll scan the file and look for markerA followed by markerB
let firstMarkerIdx = -1;
let currentPos = content.indexOf('Right: Summary Card'); // Start looking after this comment
if (currentPos !== -1) {
    const subset = content.substring(currentPos);
    const mIdx = subset.indexOf(markerA);
    if (mIdx !== -1) {
        firstMarkerIdx = currentPos + mIdx;
    }
}

const componentEnd = 'export default BookAStand;';
const lastEndIdx = content.lastIndexOf(componentEnd);

if (firstMarkerIdx !== -1 && lastEndIdx !== -1) {
    const validHeader = content.substring(0, firstMarkerIdx);
    
    const finalSuffix = \`</form>
                </motion.div>
            )}
        </AnimatePresence>
    </div>
...
export default BookAStand;\`;
    ...
    const finalContent = validHeader + cleanSummaryBlock + "\\n" + finalSuffix;
    fs.writeFileSync(filePath, finalContent);
    console.log('Public BookAStand.tsx Summary RE-CLEANED with standard whitespaces.');
} else {
