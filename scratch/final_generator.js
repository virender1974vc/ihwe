...
const tailScript = `
const fs = require('fs');
const content = fs.readFileSync('frontend/src/pages/BookAStand.tsx', 'utf8');
const lines = content.split('\\n');
const uiIdx = lines.findIndex(l => l.includes('return (') && l.includes('<div className="min-h-screen'));
const uiPart = lines.slice(uiIdx).join('\\n');
fs.writeFileSync('frontend/src/pages/BookAStand.tsx', head + body1 + uiPart);
`;

// I'll execute a node script to gather tail and write.
