...
const target = `<p className="text-2xl font-black text-[#23471d]">{formData.participation.currency === 'INR' ? \`\\u20b9\${formData.participation.total?.toLocaleString()}\` : \`\$\${formData.participation.total?.toLocaleString()}\`}</p>
                            </div>
                        </div>`;

const newUI = `<p className="text-2xl font-black text-[#23471d]">{formData.participation.currency === 'INR' ? \`\\u20b9\${formData.participation.total?.toLocaleString()}\` : \`\$\${formData.participation.total?.toLocaleString()}\`}</p>
                            </div>
                        </div>

                        {/* PAYMENT PLAN SELECTION (ADDED) */}
                        <div className="p-4 bg-slate-50 border border-slate-200 rounded-[2px] mt-3">
                             <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                                <div className="flex-1">
                                    ...
                                    <select 
                                        ...
                                        className="rounded-[2px] border border-slate-400 h-9 focus:border-[#23471d] outline-none px-3 w-full text-[12px] font-bold text-[#23471d]"
                                    >
                                        {(() => {
                                            const ev = events.find(bev => bev._id === selectedEventId);
