...
// PART 1: Imports and Initial Data (Lines 1 to 333 - end of OTP timer)
const part1 = lines.slice(0, 332).join('\n');

// PART 2: Rate Calculation and Financial Logic
const part2 = `
    ...
    useEffect(() => {
        ...
        const selectedPlan = (currentEvent?.paymentPlans || []).find((p) => p.id === plan);
...
// PART 3: Handlers (extracted from a clean portion or rewritten)
const part3 = `
...
// PART 4: The UI (Return statement with Summary) - extracted from the end of the file or rewritten
const part4 = \`
    return (
        <div className="min-h-screen bg-[#FDFDFD] text-slate-900 font-inter">
            ...
            {/* Re-using the same UI structure but ensure it is closed correctly */}
\`;

// I'll take the UI part from the end of the file, starting from the first "return ("
const returnPos = content.indexOf('return (');
const uiPartRaw = content.substring(returnPos + 8, content.lastIndexOf('</section>'));

// RECONSTRUCT
const finalTSX = part1 + part2 + part3 + part4 + uiPartRaw + \`
                </section>
            </div>
        );
    };
...
fs.writeFileSync(filePath, finalTSX);
console.log('Master Reconstruct Complete.');
