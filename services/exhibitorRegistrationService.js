...
class ExhibitorRegistrationService {
    ...
    // Called when a Proforma Invoice (an Estimate document — est_no format
    // "NGW/{FY}/PI/xxx") is created. Estimate already carries its own
    // exhibitorRegistrationId field (see models/Estimate.js), so linking is
    // per-estimate: if a registration already exists for this company+event, this
    // estimate is just linked to it; only the first estimate for a given
    // company+event actually creates one. That keeps "one registration per
    // company per event" true even when a company gets several estimates/revisions
    // for the same booking.
    async createOrLinkExhibitorRegistrationForEstimate(estimate) {
        const ExhibitorRegistration = require('../models/ExhibitorRegistration');
        const Company = require('../models/Company');
        const CrmEvent = require('../models/CrmEvent');
        const Counter = require('../models/visitor/CounterModel');

        if (!estimate?.companyId) return null;
        if (estimate.exhibitorRegistrationId) return null; // already linked

        // Estimate.eventId is rarely populated in practice — the real link is via
        // crmEventId -> CrmEvent.registrationEventId, which points at the actual
        // Event document that ExhibitorRegistration.eventId (required) needs.
        let eventId = estimate.eventId || null;
        if (!eventId && estimate.crmEventId) {
            const crmEvent = await CrmEvent.findById(estimate.crmEventId).select('registrationEventId').lean();
            eventId = crmEvent?.registrationEventId || null;
        }
        if (!eventId) return null;

        const company = await Company.findById(estimate.companyId);
        if (!company) return null;

        // One registration per company per event — reuse it if it already exists
        // (e.g. a second estimate/revision for the same booking) instead of creating
        // a duplicate; just link this estimate to the existing registration.
        let registration = await ExhibitorRegistration.findOne({
            eventId,
            clientId: String(company._id)
        });

        if (registration) {
            estimate.exhibitorRegistrationId = registration._id;
            await estimate.save();
            return registration;
        }
        ...
        let registrationId;
        while (true) {
            const candidateId = `9IHWE-EX-${year}-${currentSeq.toString().padStart(4, '0')}`;
            const existingDoc = await ExhibitorRegistration.findOne({ registrationId: candidateId }).select('_id').lean();
            if (!existingDoc) { registrationId = candidateId; break; }
            currentSeq++;
        }

        const [firstName, ...rest] = String(estimate.consignee_person || '').trim().split(/\s+/).filter(Boolean);
        const item = (estimate.items || [])[0] || {};

        registration = new ExhibitorRegistration({
            eventId,
            clientId: String(company._id),
            registrationSource: 'admin',
            registrationId,
            exhibitorName: estimate.company_name || company.companyName,
            address: estimate.company_addr || company.address,
            country: estimate.country || company.country,
            state: estimate.state || company.state,
            city: estimate.city || company.city,
            pincode: estimate.pincode ? String(estimate.pincode) : company.pincode,
            companyEmail: company.email,
            gstNo: estimate.company_gst_no || estimate.gst_no || company.gstNumber,
            panNo: company.panNo,
            contact1: {
                firstName: firstName || company.companyName,
                lastName: rest.join(' '),
                email: company.email,
                mobile: estimate.consignee_phone || ''
            },
            participation: {
                stallSize: Number(item.size) || 0,
                rate: Number(item.rate) || 0,
                amount: Number(item.amount) || 0,
                total: Number(item.finalAmount) || Number(estimate.finalAmount) || 0,
                gstPercent: parseFloat(item.gstRate) || 18,
                stallType: item.description || ''
            },
            status: 'pending',
            paymentMode: 'manual'
        });
        ...
        try {
            ...
            // Concurrent estimate creations for the same company/event can both pass
            // the findOne check above before either has saved; the partial unique
            // index on (eventId, clientId) is the real backstop — if that's what
            // tripped, just re-fetch and link to whichever one won instead of failing.
            if (saveErr.code === 11000) {
                registration = await ExhibitorRegistration.findOne({ eventId, clientId: String(company._id) });
                if (!registration) throw saveErr;
            } else {
                throw saveErr;
            }
        }
        ...
        company.exhibitorRegistrationId = registrationId;
        await company.save();
